...
train_infected_x = np.array([np.array(Image.open(filename).convert("L"))/255.0 for filename in filenames_infected])
train_healthy_x = np.array([np.array(Image.open(filename).convert("L"))/255.0 for filename in filenames_healthy])
